...
**Ruta:** `/Users/wallfacer/proyectos-gitlab/pos-online`  
...
1. **`/Users/wallfacer/proyectos-gitlab/pos-online`** — versión **completa** (ya en producción, ya subida a Gitea como `alejandro-perez/pos-online` si hiciste `git push gitea`).
...
- **Jenkins** puede apuntar a `http://gitea.unclic.consulting:3000/alejandro-perez/pos-online.git` y ejecutar el Jenkinsfile que está en ese repo (el del GitLab, con prepare/build/test/package/deploy).
...
Referencias: [CONFIGURAR-GITEA-JENKINS-SEGURO-Y-COMPARTIR-USUARIOS.md](CONFIGURAR-GITEA-JENKINS-SEGURO-Y-COMPARTIR-USUARIOS.md), [DEPENDENCIA-GENERIC-MODEL-Y-CONTEXTO.md](DEPENDENCIA-GENERIC-MODEL-Y-CONTEXTO.md) (si existe en el repo).
