...
## 1. Subdominios sugeridos (plantilla)

Sustituye **&lt;TU_DOMINIO&gt;** y las IPs por las tuyas. Confirma siempre en **AWS Console → EC2** la IP pública actual.

| Servicio | Subdominio | Valor del registro A (ejemplo) | URL (patrón) |
|----------|------------|----------------------------------|--------------|
| Landing / apex | `@`, `www` | `<IP_LANDING>` | `https://<TU_DOMINIO>` |
| Jenkins | `jenkins` | `<IP_JENKINS>` | `http://jenkins.<TU_DOMINIO>:8080` |
| Gitea | `gitea` | `<IP_GITEA>` | `http://gitea.<TU_DOMINIO>:3000` |
| POS (si aplica) | `pos` | `<IP_POS>` | `http://pos.<TU_DOMINIO>:8111` |

Sin **Elastic IP**, al reiniciar una instancia puede cambiar la IP → actualiza los registros A en el DNS.
...
| Type | Host | Value | TTL |
|------|------|--------|-----|
| A Record (o A + Dynamic DNS) | jenkins | `<IP_JENKINS>` | Automatic |

**Ejemplo para Gitea:**

| Type | Host | Value | TTL |
|------|------|--------|-----|
| A Record (o A + Dynamic DNS) | gitea | `<IP_GITEA>` | Automatic |
...
### Ejemplo de HOST RECORDS (no copiar IPs — rellena las tuyas)

| Type | Host | Value | TTL | Uso |
|------|------|--------|-----|-----|
| A | @ | `<IP_LANDING>` | Automatic | Apex → landing |
| A | www | `<IP_LANDING>` | Automatic | www |
| A | jenkins | `<IP_JENKINS>` | Automatic | Jenkins |
| A | gitea | `<IP_GITEA>` | Automatic | Gitea |
| A | pos | `<IP_POS>` | Automatic | POS dedicado (opcional) |

**Correo (ej. Google Workspace):** conserva **MX** y **TXT** (SPF/DKIM) que ya tengas; no los borres al editar A.

Si un subdominio no resuelve a la EC2 correcta, revisa que el **Value** del A coincida con la IP pública actual en AWS. Ver [URLS-Y-EC2-PRUEBAS.md](URLS-Y-EC2-PRUEBAS.md).
