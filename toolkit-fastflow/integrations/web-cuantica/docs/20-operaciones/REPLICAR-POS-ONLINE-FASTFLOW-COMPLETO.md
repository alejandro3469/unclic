...
Referencia de plantilla (dominio / IPs): [INFRAESTRUCTURA-UNCLIC-ACTUAL.md](INFRAESTRUCTURA-UNCLIC-ACTUAL.md), [URLS-Y-EC2-PRUEBAS.md](URLS-Y-EC2-PRUEBAS.md).
...
- **Instancias:** fastflow-jenkins-controller (t3.micro), fastflow-gitea (t3.micro). Opcional: tercera EC2 para POS o landing estático.
...
**Instalar Jenkins** según [../30-instalacion/INSTALAR-JENKINS.md](../30-instalacion/INSTALAR-JENKINS.md) (o doc equivalente). Tras el primer arranque: unlock con la contraseña de `/var/lib/jenkins/secrets/initialAdminPassword`, completar asistente, crear usuario admin.
...
- **Configure:** Pipeline from SCM → Git → Repository URL = `http://gitea.<TU_DOMINIO>:3000/TU_USUARIO/pos-online.git` (o la URL real), Credentials = la creada, Branch = `main`, Script Path = `Jenkinsfile`.
- Guardar. **Build Now**.
...
| Plantilla infra / URLs | [INFRAESTRUCTURA-UNCLIC-ACTUAL.md](INFRAESTRUCTURA-UNCLIC-ACTUAL.md), [URLS-Y-EC2-PRUEBAS.md](URLS-Y-EC2-PRUEBAS.md) |
