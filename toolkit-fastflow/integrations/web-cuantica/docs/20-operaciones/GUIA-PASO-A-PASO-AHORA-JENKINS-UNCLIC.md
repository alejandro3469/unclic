# Guía paso a paso — Jenkins (dominio propio)
...
1. Abre en el navegador: **http://jenkins.<TU_DOMINIO>:8080** (o **http://<IP_JENKINS>:8080**).
2. ¿Qué ves?
...
2. Localiza la instancia **fastflow-jenkins-controller** (o la que uses para Jenkins). Anota la **IP pública** (cambia si no hay Elastic IP).
...
**Opción A — Ya tienes TU_USUARIO/pos-online en Gitea**

1. Abre **http://gitea.<TU_DOMINIO>:3000/TU_USUARIO/pos-online**.
...
3. Si **no** tiene Jenkinsfile (solo lo creaste vacío), tienes que subir código. En tu Mac, en el repo que tenga el POS + Jenkinsfile (por ejemplo `proyectos-gitlab/pos-online` o la carpeta `repo-pos-fastflow` del toolkit):
   ...
   git remote add gitea http://gitea.<TU_DOMINIO>:3000/TU_USUARIO/pos-online.git
...
2. En tu Mac:
   ...
   git remote add gitea http://gitea.<TU_DOMINIO>:3000/TU_USUARIO/pos-online-fastflow.git
   git branch -M main
   git push -u gitea main
   ```
   (Sustituye `TU_USUARIO/pos-online-fastflow` por tu usuario y nombre de repo.)
3. Anota la URL del repo (ej. `http://gitea.<TU_DOMINIO>:3000/TU_USUARIO/pos-online-fastflow.git`).
...
4. Rellena:
   ...
   - **Username:** `TU_USUARIO` (tu usuario Gitea)
...
4. En la configuración del job:
   ...
   - **Repository URL:** la URL de tu repo en Gitea, por ejemplo:
     ...
     - o `http://gitea.<TU_DOMINIO>:3000/TU_USUARIO/pos-online-fastflow.git`
...
5. Si el job está en rama **main** y tiene stage Deploy, la app debería quedar en el puerto **8111**. Prueba en el navegador: **http://<IP_JENKINS>:8111/health** (o la IP de tu EC2).
