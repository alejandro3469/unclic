...
- **Flujo EC2 → Jenkins con outputs reales y porqués:** [FLUJO-EC2-JENKINS-CON-OUTPUTS-Y-PORQUES.md](FLUJO-EC2-JENKINS-CON-OUTPUTS-Y-PORQUES.md) (comandos, salidas de terminal y explicación paso a paso para usuario final).  
- Conceptos y diagramas: [Capítulo 3](CAPITULO-DESPLIEGUE-OPERACION-AWS.md), [Capítulo 4](CAPITULO-SERVICIOS-CORE-AWS.md), [Capítulo 6](CAPITULO-FACTURACION-Y-PRECIOS.md).  
- Comprobar si el flujo se cumple: [VERIFICACION-FLUJO-UN-COMMIT-LEVANTA-INSTANCIA.md](VERIFICACION-FLUJO-UN-COMMIT-LEVANTA-INSTANCIA.md).
...
7. Anotar el output **jenkins_public_ip** (ej. 3.15.4.160); es la IP para acceder a Jenkins y, más adelante, a la app.
...
6. En el navegador abrir **http://&lt;jenkins_public_ip&gt;:8080** (sustituir por tu IP, ej. 3.15.4.160), pegar la contraseña en la pantalla **“Unlock Jenkins”** y completar el asistente (plugins, usuario admin).

Para ver **qué pantalla verás** y un **ejemplo de salida** del comando (estilo Jenkins/terminal), ver [JENKINS-UNLOCK-PANTALLA-Y-OUTPUT.md](JENKINS-UNLOCK-PANTALLA-Y-OUTPUT.md). Para el **flujo completo en la EC2 con outputs reales** (Java, JAVA_HOME, systemctl, contraseña inicial) y porqués, ver [FLUJO-EC2-JENKINS-CON-OUTPUTS-Y-PORQUES.md](FLUJO-EC2-JENKINS-CON-OUTPUTS-Y-PORQUES.md).
