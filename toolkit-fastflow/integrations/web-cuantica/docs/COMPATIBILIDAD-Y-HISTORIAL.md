...
| Nivel | Qué copiar |
|-------|------------|
| **Mínimo** | `Jenkinsfile.example`, `deploy/dashboard-demo-jenkins-registry.html`, `docs/pos-online/FLUJO-COMMIT-Y-USUARIO-FINAL.md`, `docs/instalacion/INSTALAR-JENKINS.md`, `scripts/validate-jenkinsfile.sh`, `scripts/test-registry.sh`. |
| **Completo** | Lo anterior + `scripts/validate-terraform.sh`, `validate-k8s.sh`, `run-all-validations.sh`, `build-and-push.sh`, `simulate-jenkins-pipeline.sh`, `deploy/terraform/`, `deploy/k8s/`, `docs/pos-online/`, `docs/DEPENDENCIA-GENERIC-MODEL-Y-CONTEXTO.md`. |
...
**Guardar en local** (desde la raíz del repo): añadir `> toolkit-fastflow/integrations/web-cuantica/docs/historial/<nombre>` al final. Ejemplo:  
`git show 5dfdc92:toolkit-fastflow/integrations/web-cuantica/docs/AGENTES-FASTFLOW-REGISTRY-EMPAQUETADO.md > toolkit-fastflow/integrations/web-cuantica/docs/historial/AGENTES-FASTFLOW-REGISTRY-EMPAQUETADO.md`

Cualquier otro archivo en ese commit: usar la ruta `toolkit-fastflow/integrations/web-cuantica/docs/<nombre>`.

Ver también **docs/historial/README.md** en este repo.
