...
Checklist para poder ejecutar el pipeline pos-online en **Jenkins** (http://jenkins.<TU_DOMINIO>:8080 o https://jenkins.<TU_DOMINIO>).
...
|------|-----------|------------|
| Conectar a la EC2 | AWS Console → EC2 → fastflow-jenkins-controller → Connect (EC2 Instance Connect) | [PASO-A-PASO-MINIMO-HOY.md](../20-operaciones/PASO-A-PASO-MINIMO-HOY.md) § 1 |
| Java 17 + Jenkins + Maven | En la EC2: instalar Corretto 17, Jenkins, Maven; `systemctl start jenkins` | § 2 |
| Desbloquear Jenkins | `sudo cat /var/lib/jenkins/secrets/initialAdminPassword` → pegar en http://<IP_JENKINS>:8080 → Install suggested plugins → crear admin | § 3 |
| Puerto 8111 (app POS) | EC2 → Security Groups → fastflow-jenkins-sg → Add rule TCP 8111 | § 4 |

**Comprobar:** Abres http://jenkins.<TU_DOMINIO>:8080 (o http://<IP_JENKINS>:8080) y entras al dashboard.
...
2. **Configure:**
   ...
   - **SCM:** Git.
   - **Repository URL:** `http://gitea.<TU_DOMINIO>:3000/alejandro-perez/pos-online.git` (o la URL de tu repo en Gitea).
...
- En la EC2 de Jenkins: Nginx + `certbot --nginx -d jenkins.<TU_DOMINIO>` ([HTTPS-UNCLIC-GITEA-JENKINS.md](../20-operaciones/HTTPS-UNCLIC-GITEA-JENKINS.md)).
- En Jenkins: **Manage Jenkins** → **System** → **Jenkins URL:** `https://jenkins.<TU_DOMINIO>/`.
...
1. [ ] Jenkins instalado y accesible (http://jenkins.<TU_DOMINIO>:8080).
...
Referencias: [CONFIGURAR-GITEA-JENKINS-SEGURO-Y-COMPARTIR-USUARIOS.md](../20-operaciones/CONFIGURAR-GITEA-JENKINS-SEGURO-Y-COMPARTIR-USUARIOS.md), [PASO-A-PASO-MINIMO-HOY.md](../20-operaciones/PASO-A-PASO-MINIMO-HOY.md), [DEPENDENCIA-GENERIC-MODEL-Y-CONTEXTO.md](../20-operaciones/DEPENDENCIA-GENERIC-MODEL-Y-CONTEXTO.md).
