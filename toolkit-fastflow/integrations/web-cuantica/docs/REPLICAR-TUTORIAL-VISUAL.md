...
Cuando tengas **assets** (capturas, HTML de pantallas), súbelos a `docs/tecnologias/assets/` o `docs/assets/replicar/` y sustituye los placeholders `[ASSET: nombre]` por la ruta real (p. ej. `![Consola EC2](tecnologias/assets/aws-ec2-instances.png)` o un enlace a un fragmento HTML en `deploy/replicar-tutorial-visual.html`).
...
| **Comando / Acción** | Añadir o editar registro **A**: Host = `jenkins`, Value = `<IP-EC2-Jenkins>` (ej. 18.218.37.76). Repetir para `gitea`, `pos`, `@`/`www` según [DOMINIO-NAMECHEAP-UNCLIC-EC2.md](DOMINIO-NAMECHEAP-UNCLIC-EC2.md). |
...
# Arrancar Jenkins; luego en navegador abrir :8080</pre> Ver [instalacion/INSTALAR-JENKINS.md](instalacion/INSTALAR-JENKINS.md) para la secuencia exacta. |
...
| **Ayuda visual** | [ASSET: jenkins-new-item] — Pantalla New Item: campo "Enter an item name", lista "Pipeline", "Freestyle project", etc. Botón **OK**. Ver [JENKINS-JOB-PANTALLAS-NEW-ITEM-Y-CONFIGURE.md](JENKINS-JOB-PANTALLAS-NEW-ITEM-Y-CONFIGURE.md). |
...
- **Imágenes (capturas):** `docs/tecnologias/assets/` — p. ej. `aws-ec2-instances-table.png`, `jenkins-unlock-screen.png`, `namecheap-host-records.png`. En este doc, reemplaza `[ASSET: nombre]` por `![Descripción](tecnologias/assets/nombre.png)`.
...
- **Lista de inputs/comandos ejecutados:** Los comandos ya están en las tablas de cada paso. Para una tabla maestra "todos los comandos en orden", ver sección equivalente en [DOCUMENTACION-POR-TECNOLOGIA-Y-REPLICACION.md](DOCUMENTACION-POR-TECNOLOGIA-Y-REPLICACION.md) y los docs en [tecnologias/](tecnologias/).
...
IPs actuales y más URLs: [URLS-Y-EC2-PRUEBAS.md](URLS-Y-EC2-PRUEBAS.md).
