package mx.com.endtoend.infrastructure.services.jde.catalogue.calzada.common.repository;
...
import mx.com.endtoend.infrastructure.services.jde.catalogue.calzada.common.entities.F0005Id;
...
@Repository
public interface F0005Repository extends JpaRepository<F0005, F0005Id>{
	...
	List<F0005> findByDrsyAndDrrtAndDrdl01(@Param("drsy") String drsy, @Param("drrt") String drrt, @Param("drl01") String drl01);

	@Query("FROM F0005 f WHERE TRIM(f.id.drsy) =:paramOne AND TRIM(f.id.drrt) =:paramTwo AND TRIM(f.id.drky) =:drky")
	F0005 findDescriptionByDrsyAndDrrtAndDrky(@Param("paramOne") String drsy, @Param("paramTwo") String drrt, @Param("drky") String drky);

	@Query("FROM F0005 f WHERE TRIM(f.id.drsy) = :drsy AND TRIM(f.id.drrt) = :drrt  AND TRIM(f.drsphd) = :drsphd  ORDER BY f.drdl01 ASC")
	List<F0005> findByDrsyAndDrrtAndDrsphd(@Param("drsy") String drsy, @Param("drrt") String drrt, @Param("drsphd") String drsphd);
	...
	F0005 findByDrsyAndDrrtAndDrsphdDrdl02(@Param("drsy") String drsy, @Param("drrt") String drrt, @Param("drsphd") String drsphd, @Param("drdl02") String drdl02);
