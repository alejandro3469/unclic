...
import mx.com.endtoend.infrastructure.articles.demo.oracle.entities.F41021;
import mx.com.endtoend.infrastructure.articles.demo.oracle.entities.F41021Id;

@Repository
public interface F41021DemoRepository extends JpaRepository<F41021, F41021Id> {
	...
	double getAvailabilityByArticleNumberAndWarehouseCode(@Param("articleCode") BigDecimal articleCode,
														  @Param("warehouseCode") String warehouseCode);

	@Transactional
	@Modifying
	@Query("UPDATE F41021 f " + "set f.liot1p =:ot1p + liot1p, " + "f.lipid =:pid, " + "f.liupmj =:upmj, "
			+ "f.liuser =:user, " + "f.litday =:tday " + "WHERE " + "f.id.liitm =:itm " + "AND "
			+ "TRIM(f.id.limcu) =:mcu ")
