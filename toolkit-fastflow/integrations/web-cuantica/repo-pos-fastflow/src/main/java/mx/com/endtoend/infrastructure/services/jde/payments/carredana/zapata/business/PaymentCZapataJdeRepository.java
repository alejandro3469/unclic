package mx.com.endtoend.infrastructure.services.jde.payments.carredana.zapata.business;
...
import mx.com.endtoend.infrastructure.services.jde.payments.carredana.common.entities.F4211;
...
public class PaymentCZapataJdeRepository implements GenericPaymentJdeRepository {

	@Autowired
	private OrderFCaredanaJedConverter orderJDEConverter;

	@Autowired
	private OrderCZapataJdeRepository orderCalzadaJdeRepository;
	...
	@Value("${app.zapata.queue.jde.order.name}")
	private String queueCreateOrder;

	@Value("${app.zapata.queue.jde.credit.note.name}")
	private String queueCreateCreditNote;

	@Value("${app.zapata.queue.jde.invoice.name}")
	private String queueCreateInvoiceRecord;

	private final Logger LOG = LoggerFactory.getLogger(PaymentCZapataJdeRepository.class);
	...
	public void updateItemAvailavility(OrderDetailDto orderDetailDto) {
		...
		// Validar que el número de artículo sea positivo
		if (orderDetailDto.getArticleNumber() == null || orderDetailDto.getArticleNumber().compareTo(BigDecimal.ZERO) <= 0) {
			throw new IllegalArgumentException("ARTICLE NUMBER MUST BE GREATER THAN ZERO");
		}
