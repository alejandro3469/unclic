 ...
 * su relación con el sistema JD Edwards y el sistema principal Smart Business
 * 
 * @author ddcasas
 */
public class OrderBusinessMethodOne implements OrderInterface {
	...
	public OrderBusinessMethodOne(CustomInterfaceOrderParams customInterfaceOrderParams) {
		...
		this.orderLogServicePort = customInterfaceOrderParams.getOrderLogServicePort();
		this.companyCode = customInterfaceOrderParams.getCompanyCode();
		this.idOperation = customInterfaceOrderParams.getIdOperation();
		this.userLogged = customInterfaceOrderParams.getUserLogged();
		this.rolePersistencePort = customInterfaceOrderParams.getRolePersistencePort();
		this.permissionPersistencePort = customInterfaceOrderParams.getPermissionPersistencePort();
	...
	public ResponseModel updateOrder(OrderDto orderDto, Boolean mailUsuario) {
		...
		LOG.info("{} SAVE LOG IN DB", idOperation);
		orderLogServicePort.generateOrderChangeLog(companyCode, userLogged, orderSaved, orderDto);
	...
	private ResponseModel handleActiveTime(OrderDto orderDto, EmployeeDto employeeDto, BigDecimal orderNumber,
		...
		if (true) { // TODO: Fix validation if necessary
	...
	public ResponseModel approveOrder(GenericActionControllOrderDto genericActionControllOrderDto) {
		...
		if (employeeDto == null)
         ...
         List<PermissionDto> permissionDtoList = permissionPersistencePort.findAllByUserId(employeeDto.getUserId());
		if (permissionDtoList == null) {
			throw new ValidationError("USER-CONFIGURATION-PERMISSIONS-INCOMPLETE");
		} 
		
		LOG.info("{} PERMISSION LIST SIZE: {}", idOperation, permissionDtoList.size());
		boolean containsPermission = permissionDtoList.stream()
		.anyMatch(p -> PermissionEnum.ORDERS_APPROVE.toString().equals(p.getName()));

		if (!containsPermission) {
			LOG.warn("{} USER NOT HAVE PERMISSION TO APPROVE ORDER", idOperation);
			throw new ValidationError("USER NOT HAVE PERMISSION TO APPROVE ORDER");
		}
	...
	private void validOperativeData(GenericActionControllOrderDto genericActionControllOrderDto, OrderDto orderDetail,
		...
		if (!validationDiscount.isEmpty())
			throw new ValidationError("Porcentaje de descuento excede del permitido");
	}
	...
	public ResponseModel getOrderListByParams(GenericSerchParamsOrderDto params, String email) {
		...
		try {
			...
			case OPERATIONAL:
				...
				break;
			case MANAGEMENT:
				LOG.info("{} USER HAS ROLE MANAGEMENT", idOperation);
				params.setIdUser(employeeDto.getUserId());
				break;
			case SUPERVISION_I:
	...
	public ResponseModel getAllOrdersListByCompany(GenericSerchParamsOrderDto params, String email) {
		...
		List<OrderDto> orderDtoList = (List<OrderDto>) orderPersistencePort
				.getAllOrdersListByCompany(params, companyCode, "", idOperation).getData();
	...
	public ResponseModel viewOderDetailByOrderNumber(BigDecimal orderNumber, String orderType) {
		LOG.info(String.format("%s INIT viewOderDetailByOrderNumber() ", idOperation));
		OrderDto orderDto = (OrderDto) orderPersistencePort
				.vieworderDetailByOrderNumber(orderNumber, orderType, companyCode, idOperation).getData();
		if (orderDto == null)
	...
	public ResponseModel viewHistoricalOrderDetailByOrderNumber(BigDecimal orderNumber, String orderType) {
		LOG.info(String.format("%s INIT viewHistoricalDetailQuoteOrder() ", idOperation));
	...
	public ResponseModel generateTicketByOrderNumberAndOrderType(BigDecimal orderNumber, String orderType) {
		...
		// TODO: Fix dependencies (payment) to access this method
