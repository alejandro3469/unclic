...
import mx.com.endtoend.infrastructure.services.jde.catalogue.calzada.common.entities.F0117Id;



@Repository
public interface F0117Repository extends JpaRepository<F0117, F0117Id>, CustomDirectionRepository {
	...
	//@Query(value = "select * from PRODDTA.F0117 inner join PRODDTA.F0118 on A8ADDZ = A7ADDZ where (trim(A7ADD4)) = :colony and (trim(A8ADDZ)) = :cp", nativeQuery = true)
	...
	//TODO: CAMBIAR CUANDO SE PASE A PRD - BUG 
	//@Query(value = "select * from PRODDTA.F0117 inner join PRODDTA.F0118 on A8ADDZ = A7ADDZ where (trim(A8ADDS)) = :state", nativeQuery = true)
