package mx.com.endtoend.infrastructure.services.jde.payments.calzada.common.converters;
...
import mx.com.endtoend.infrastructure.services.jde.payments.calzada.common.entities.F47011Id;
...
import mx.com.endtoend.infrastructure.services.jde.payments.calzada.common.entities.F4706Id;
...
@Component
public class OrderJDEConverter {
	...
	public F47011 orderDtoToF47011(OrderDto orderDto, BigDecimal an8Branch) {
		...
		if (an8Branch == null) {
			throw new IllegalArgumentException("AN8 BRANCH CANNOT BE NULL");
		}
		
		// Validar que el número de orden sea positivo
		if (orderDto.getOrderNumber() == null || orderDto.getOrderNumber().compareTo(BigDecimal.ZERO) <= 0) {
			throw new IllegalArgumentException("ORDER NUMBER MUST BE GREATER THAN ZERO");
		}
		
		// Validar que la tasa de cambio sea positiva
		if (orderDto.getExchangeRate() == null || orderDto.getExchangeRate().compareTo(BigDecimal.ZERO) <= 0) {
		...
		f47011.setSycrr((long) (orderDto.getExchangeRate().multiply(BigDecimal.valueOf(100)).longValue()));
		...
		f47011.setSyptc("C02");
		f47011.setSyryin(""); // TODO: CAMPO CON DUDA
		f47011.setSyrcd(" ");
	...
	public F47012 orderDetailDtoToF47012(OrderDetailDto orderDetailDto, OrderDto orderDto) {
		...
		if (orderDetailDto == null) {
			throw new IllegalArgumentException("ORDER DETAIL DTO CANNOT BE NULL");
		}
		if (orderDto == null) {
			throw new IllegalArgumentException("ORDER DTO CANNOT BE NULL");
		}
		...
		if (orderDetailDto.getRequestAmount() == null || orderDetailDto.getRequestAmount().compareTo(BigDecimal.ZERO) <= 0) {
			throw new IllegalArgumentException("REQUEST AMOUNT MUST BE GREATER THAN ZERO");
		}
		
		// Validar que el precio unitario final sea positivo
		if (orderDetailDto.getFinalUnitPrice() == null || orderDetailDto.getFinalUnitPrice().compareTo(BigDecimal.ZERO) <= 0) {
			throw new IllegalArgumentException("FINAL UNIT PRICE MUST BE GREATER THAN ZERO");
		}
		...
		f47012.setSzsoqs((long) (orderDetailDto.getRequestAmount().multiply(BigDecimal.valueOf(1000)).longValue()));
		...
		f47012.setSzuprc((long) (orderDetailDto.getFinalUnitPrice().multiply(BigDecimal.valueOf(10000)).longValue()));
		f47012.setSzdct("");
		...
		f47012.setSzuorg((long) (orderDetailDto.getRequestAmount().multiply(BigDecimal.valueOf(1000)).longValue()));
		f47012.setSzurcd("CP");
		f47012.setSzurdt(0L);
		f47012.setSzurat(0L);
		f47012.setSzurab(urab);
		f47012.setSzurrf("               ");
		f47012.setSzogno((long) (orderDetailDto.getRequestAmount().multiply(BigDecimal.valueOf(1000)).longValue()));
