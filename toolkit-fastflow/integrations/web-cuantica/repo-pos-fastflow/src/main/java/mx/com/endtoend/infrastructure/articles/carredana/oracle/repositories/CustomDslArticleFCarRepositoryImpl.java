...
import mx.com.endtoend.infrastructure.articles.carredana.oracle.entities.QF4101;
import mx.com.endtoend.infrastructure.articles.carredana.oracle.entities.QF4102;
import mx.com.endtoend.infrastructure.articles.carredana.oracle.entities.QF41021;
import mx.com.endtoend.infrastructure.articles.carredana.oracle.entities.QF4106;

@Repository
@ConditionalOnProperty(name = "app.carredana.oracle.enabled", havingValue = "true", matchIfMissing = false)
public class CustomDslArticleFCarRepositoryImpl extends QuerydslRepositorySupport {

	@Autowired
	@Qualifier("carredanaOracleEntityManagerFactory")
	EntityManager em;

	public CustomDslArticleFCarRepositoryImpl() {
		super(ArticleDto.class);
	}

	private final Logger LOG = LoggerFactory.getLogger(CustomDslArticleFCarRepositoryImpl.class);
	...
	public List<ArticleDto> findArticlesByParams(GenericSerchParamsArticleDto serrchParams) {
		...
		if (serrchParams.getArticleCode() != null) {
			if (!serrchParams.getArticleCode().isEmpty()) {
				...
				query.where(
						F4101.imlitm.like("%" + stringUtil.cleanStringToQuery(serrchParams.getArticleCode()) + "%"));
		 ...
		 * Select fields
		 */
		query.select(Projections.constructor(ArticleDto.class,

				F4106.id.bpcgid, F4102.id.ibitm, F4102.id.ibmcu, F4102.ibvend, F4102.ibaitm, F4102.ibprgr, F4102.iblnty,

				F4101.imdsc1, F4101.imdsc2, F4101.imlitm, F4101.imprp2, F4102.ibstkt, // AJUSTE EN MAPEO
				F4101.imsrp1, F4101.imsrp3, F4101.imsrp4, F4101.imsrp5, F4101.imsrtx, F4101.imuom1, F4101.imuom9,
