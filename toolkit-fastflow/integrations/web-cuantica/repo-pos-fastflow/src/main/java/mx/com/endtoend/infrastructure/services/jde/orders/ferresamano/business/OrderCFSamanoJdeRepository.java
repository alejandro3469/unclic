package mx.com.endtoend.infrastructure.services.jde.orders.ferresamano.business;
...
import mx.com.endtoend.infrastructure.services.jde.orders.ferresamano.common.entities.F00021;
import mx.com.endtoend.infrastructure.services.jde.orders.ferresamano.common.repository.F00021FSamanoRepository;
...
public class OrderCFSamanoJdeRepository implements GenericOrderJdeRepository {

	@Autowired
	private F00021FSamanoRepository f00021Repository;
	...
	private final Logger LOG = LoggerFactory.getLogger(OrderCFSamanoJdeRepository.class);
	...
	private String companyKey = "00010";
	...
	public BigDecimal getOrderNumberByOrderType(String orderType, String idOperation) {
		...
		try {
			...
			if (f00021 != null) {
				...
				// Para FERRESAMANO, retornamos un número por defecto si no existe en la BD
				orderNumber = new BigDecimal("10001");
				LOG.info(String.format("%s FERRESAMANO ORDER-NUMBER (default): " + orderNumber.toString(), idOperation));
			}
	...
	public BigDecimal getOrderNumberByExternalServiceAndOrderTypeAndCompanyNumber(String orderType,
		...
		try {
			...
			LOG.info(String.format("%s FERRESAMANO ORDER-NUMBER: " + orderNumber.toString(), idOperation));
			LOG.info(String.format("%s RETURN ORDER-NUMBER", idOperation));
			return orderNumber;
		} catch (Exception e) {
	...
	public boolean getIsItemAvailabilityByParams(BigDecimal articleNumber, BigDecimal requestAmount, String unitMeasurement,
		...
		try {
			...
			if (quantity != null && quantity.compareTo(BigDecimal.ZERO) > 0 && quantity.compareTo(requestAmount) >= 0) {
