package mx.com.endtoend.domain.catalogue.bussiness.branch;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import mx.com.endtoend.domain.commons.constants.GenericIdentifyMethods;
import mx.com.endtoend.infrastructure.services.jde.catalogue.common.serviceport.CatalogueJdeServicePort;

public class CatalogueBranchFactory {

	private final static Logger LOG = LoggerFactory.getLogger(CatalogueBranchFactory.class);

	public CatalogueBranchInterface getImplementationByCode(String method,
			CatalogueJdeServicePort catalogueOracleServicePort) {
		try {
			LOG.info(String.format("INIT getImplementationByCode() "));
			GenericIdentifyMethods value = GenericIdentifyMethods.valueOf(method);
			switch (value) {
			case CAT_BRANCH_ONE:
				return new CatalogueBranchMethodOne(catalogueOracleServicePort);
			default:
				return null;
			}
		} catch (IllegalArgumentException | NullPointerException ex) {
			LOG.error("ERROR GETTING METHOD TYPE FOR CATALOGUE-BRANCH MODULE");
			return null;
		}
	}
}