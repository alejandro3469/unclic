package mx.com.endtoend.infrastructure.services.posLegacy.orders.calzada.fragua.business;
...
import mx.com.endtoend.infrastructure.services.posLegacy.orders.calzada.fragua.repositories.DetalleSolTrasCFraguaRepository;
import mx.com.endtoend.infrastructure.services.posLegacy.orders.calzada.fragua.repositories.DirEnvioClienteOrdenesCFraguaRepository;
import mx.com.endtoend.infrastructure.services.posLegacy.orders.calzada.fragua.repositories.EncabezadoSolTraspasoCFraguaRepository;
import mx.com.endtoend.infrastructure.services.posLegacy.orders.calzada.fragua.repositories.TblDetalleOrdenCFraguaBkupRepository;
import mx.com.endtoend.infrastructure.services.posLegacy.orders.calzada.fragua.repositories.TblDetalleOrdenCFraguaRepository;
import mx.com.endtoend.infrastructure.services.posLegacy.orders.calzada.fragua.repositories.TblEncabezadoOrdenCFraguaBkupRepository;
import mx.com.endtoend.infrastructure.services.posLegacy.orders.calzada.fragua.repositories.TblEncabezadoOrdenCFraguaRepository;
import mx.com.endtoend.infrastructure.services.posLegacy.orders.calzada.fragua.repositories.UsuariosCFraguaRepository;
...
@Service
public class OrderPosLegacyCFraguaRepository implements GenericOrderPosLegacyRepository {

	@Autowired(required = false)
	private TblEncabezadoOrdenCFraguaRepository tblEncabezadoOrdenRepository;

	@Autowired(required = false)
	private TblEncabezadoOrdenCFraguaBkupRepository tblEncabezadoOrdenBkupRepository;

	@Autowired(required = false)
	private TblDetalleOrdenCFraguaRepository tblDetalleOrdenRepository;

	@Autowired(required = false)
	private TblDetalleOrdenCFraguaBkupRepository tblDetalleOrdenBkupRepository;

	@Autowired(required = false)
	private DirEnvioClienteOrdenesCFraguaRepository dirEnvioClienteOrdenesRepository;
	...
	@Autowired(required = false)
	private EncabezadoSolTraspasoCFraguaRepository encabezadoSolTraspasoRepository;

	@Autowired(required = false)
	private DetalleSolTrasCFraguaRepository detalleSolTrasRepository;

	@Autowired(required = false)
	private UsuariosCFraguaRepository usuariosRepository;
	...
	@Value("${app.fragua.queue.order.name}")
	private String queueCreateOrder;

	@Value("${app.fragua.queue.oreder.direct.name}")
	private String queueCreateOrderDirect;

	@Value("${app.fragua.queue.approve.oreder.name}")
	private String queueApproveOrder;

	@Value("${app.fragua.queue.cancel.oreder.name}")
	private String queueCancelOrder;

	@Value("${app.fragua.queue.status.oreder.name}")
	private String queueUpdateStatus;

	private final Logger LOG = LoggerFactory.getLogger(OrderPosLegacyCFraguaRepository.class);

	private String companyKey = "00009";

	String usernamePL = "";

	/**
	 * Método para la creación de ordenes del sistema en la BD SQL Server
	 * 
	...
	public void sendOrderODToSave(SaleOrderDto saleOrderDto) {
		try {
			...
			LOG.error(String.format("ERROR IN sendOrderODToSave() : Exception: %s", e.getMessage()));
		}
	}
