...
public class ClientMethodOne implements ClientInterface {

	private final static Logger LOG = LoggerFactory.getLogger(ClientMethodOne.class);
	private final ClientOracleServicePort clientOracleServicePort;
	private final GenericIdentifyMethods method = GenericIdentifyMethods.CLI_CONF_ONE;
	public GenericValidationToClient genericValidationToClient = new GenericValidationToClient();

	public ClientMethodOne(ClientOracleServicePort clientOracleServicePort) {
	...
	public ResponseModel getListClient(FiltersClientDto filtersClientDto, String companyCode,
		...
		if (!validInputParams.isEmpty()) {
			LOG.warn(String.format("%s EMPTY PARAMS TO SERCH CLIENTS", idOperation));
	...
	public ResponseModel createClient(ClientDto clientDto, String companyCode,
		...
		if (clientDto.getNoClient() == null) {
			LOG.info(String.format("%s GET CLIENTE NUMBER FROM JDE SERVICE", idOperation));
		...
		LOG.info("SE CREA EL CLIENT LOCAL: " + clientDto.getName() + clientDto.getFatherSurname()
	...
	public ResponseModel updateClient(ClientDto clientDto, String companyCode,
		...
		LOG.info(String.format("%s SEND CLIENT TO UPDATE", idOperation));
		...
		LOG.info(String.format("%s CALL JDE SERVICE TO UPDATE NEW CLIENT", idOperation));
