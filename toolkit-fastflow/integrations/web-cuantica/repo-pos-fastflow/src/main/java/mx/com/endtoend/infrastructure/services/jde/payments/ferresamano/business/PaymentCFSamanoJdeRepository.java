package mx.com.endtoend.infrastructure.services.jde.payments.ferresamano.business;
...
import mx.com.endtoend.infrastructure.services.jde.payments.ferresamano.common.converters.OrderCFSamanoJedConverter;
...
import mx.com.endtoend.infrastructure.services.jde.payments.ferresamano.common.entities.F4706;
import mx.com.endtoend.infrastructure.services.jde.payments.ferresamano.common.entities.F4714;
import mx.com.endtoend.infrastructure.services.jde.payments.ferresamano.common.repository.F47011FSamanoRepository;
import mx.com.endtoend.infrastructure.services.jde.payments.ferresamano.common.repository.F47012FSamanoRepository;
import mx.com.endtoend.infrastructure.services.jde.payments.ferresamano.common.repository.F4706FSamanoRepository;
import mx.com.endtoend.infrastructure.services.jde.payments.ferresamano.common.repository.F4714FSamanoRepository;
...
public class PaymentCFSamanoJdeRepository implements GenericPaymentJdeRepository {

	@Autowired
	private OrderCFSamanoJedConverter orderJDEConverter;
	...
	@Autowired
	private F47011FSamanoRepository f47011Repository;

	@Autowired
	private F47012FSamanoRepository f47012Repository;

	@Autowired
	private F4706FSamanoRepository f4706Repository;

	@Autowired
	private F4714FSamanoRepository f4714Repository;
	...
	@Value("${app.fsamano.jde.batchFolio.generator.url}")
	String urlbatchFolio;

	@Value("${app.fsamano.jde.consecutive.generator.url}")
	String urlNextOrderNumber;
	
	@Value("${app.fsamano.jde.company.code}")
	String companyCode;

	@Value("${app.fsamano.queue.jde.order.name}")
	private String queueCreateOrder;

	@Value("${app.fsamano.queue.jde.credit.note.name}")
	private String queueCreateCreditNote;

	@Value("${app.fsamano.queue.jde.invoice.name}")
	private String queueCreateInvoiceRecord;

	private final Logger LOG = LoggerFactory.getLogger(PaymentCFSamanoJdeRepository.class);
	...
	public boolean saveOrder(OrderDto orderDto, String idOperation) {
		...
		try {
			...
			BigDecimal genericClientNumber = orderCFSamanoJdeRepository.getGenericClientNumberByWarehouseCode(
	...
	public List<InvoiceRecordDto> searchInvoiceRecordsByOrden(OrderDto orderDto,
		...
		try {
			...
			// HOMOLOGACIÓN: Usar F47012 igual que DEMO y Carredana (en lugar de F4211/F42119)
			for (OrderDetailDto orderDetailDto : orderDetailList) {
				...
				List<F47012> f47012SearchList = f47012Repository.findAllByKooAndDocoAndDctoAndItm(
						orderDto.getCompanyNumber(), 
						orderDto.getOrderNumber().longValue(), 
