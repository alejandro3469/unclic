package mx.com.endtoend.infrastructure.services.jde.payments.calzada.business;
...
public class PaymentCalzadaJdeRepository implements GenericPaymentJdeRepository {
...
//	private F42119Repository f42119Repository;
	...
	@Value("${app.calzada.queue.jde.order.name}")
	...
	@Value("${app.calzada.queue.jde.credit.note.name}")
	private String queueCreateCreditNote;

	@Value("${app.calzada.queue.jde.invoice.name}")
	private String queueCreateInvoiceRecord;

	private final Logger LOG = LoggerFactory.getLogger(PaymentCalzadaJdeRepository.class);
	...
	public void resendNotesToQueue(PaymentOrderJDE paymentOrderJDE) {
		try {
			if (rabbitTemplate == null) {
				LOG.warn("RabbitTemplate is not available. Notes will not be resent to RabbitMQ queue.");
				return;
			}
			// Validar precisión decimal antes de enviar
			validatePaymentOrderPrecision(paymentOrderJDE);
			...
			LOG.info(String.format("Resending notes to RabbitMQ with precision validation: OrderTotal=%s", 
					paymentOrderJDE.getOrder().getOrderTotal()));
			
			rabbitTemplate.convertAndSend(queueCreateCreditNote, paymentOrderJDE);
			
			LOG.info("Notes successfully resent to RabbitMQ queue: " + queueCreateCreditNote);
		} catch (Exception e) {
			LOG.error(String.format("ERROR IN resendNotesToQueue() : Exception: %s", e.getMessage()));
		}
	}
