...
export function Orb({
  ...
  return (
    <div className={cn('relative h-full w-full min-h-[280px]', className)}>
      <Canvas
...
function OrbScene({
  ...
  useEffect(() => {
    const apply = () => {
      ...
      const mat = circleRef.current.material as THREE.ShaderMaterial;
      if (mat?.uniforms?.uInverted) {
...
void main() {
  ...
  vec3 ramp1 = vec3(0.0, 0.0, 0.0);
  vec3 ramp2 = uColor1;
  vec3 ramp3 = uColor2;
  vec3 ramp4 = vec3(1.0, 1.0, 1.0);
  float luminance = mix(color.r, 1.0 - color.r, uInverted);
  color.rgb = colorRamp(luminance, ramp1, ramp2, ramp3, ramp4);
  color.a *= uOpacity;
  gl_FragColor = color;
}
`;

/** Controles de estado del agente para el Orb (Idle, Listening, Talking, Thinking). */
export function OrbAgentStateControls({
  value,
  onChange,
}: {
  value: AgentState;
  ...
  const states: { label: string; state: AgentState }[] = [
    { label: 'Idle', state: null },
    { label: 'Listening', state: 'listening' },
    { label: 'Talking', state: 'talking' },
